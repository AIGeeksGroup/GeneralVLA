...
def get_qwen_from_pretrained_kwargs(args):
    custom_kwargs = getattr(args, "robot_memory_vla_qwen_load_kwargs", None)
    if custom_kwargs is None:
        return {"device_map": "cuda"}
    return dict(custom_kwargs)


def get_qwen_cpu_fallback_kwargs(args):
    custom_kwargs = getattr(args, "robot_memory_vla_qwen_cpu_fallback_kwargs", None)
    if custom_kwargs is None:
        return None
    return dict(custom_kwargs)


def load_qwen_model(model_cls, model_path, args):
    model_load_kwargs = get_qwen_from_pretrained_kwargs(args)
    try:
        ...
        fallback_kwargs = get_qwen_cpu_fallback_kwargs(args)
...
class GroundingModel_Qwen_full(): 
    def __init__(self, model_path, args):
        ...
        self.model = load_qwen_model(AutoModelForCausalLM, model_path, args)
...
class GroundingModel_Qwen_lora():
    ...
    def __init__(self, model_path, args):
        ...
        self.model = load_qwen_model(AutoPeftModelForCausalLM, model_path, args)
