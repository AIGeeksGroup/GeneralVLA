...
def get_lisa_from_pretrained_kwargs(args, torch_dtype):
    kwargs = {
        "torch_dtype": torch_dtype,
        "low_cpu_mem_usage": True,
    }
    custom_kwargs = getattr(args, "robot_memory_vla_lisa_load_kwargs", None)
    if custom_kwargs is not None:
        kwargs.update(dict(custom_kwargs))
    return kwargs


def get_lisa_cpu_fallback_kwargs(args):
    custom_kwargs = getattr(args, "robot_memory_vla_lisa_cpu_fallback_kwargs", None)
    if custom_kwargs is None:
        return None
    return dict(custom_kwargs)


def get_lisa_retry_load_kwargs(args):
    custom_kwargs = getattr(args, "robot_memory_vla_lisa_retry_load_kwargs", None)
    if custom_kwargs is None:
        return None
    return dict(custom_kwargs)


def get_primary_device_from_model(model, requested_device):
...
class LISASegmenter:
    ...
    def __init__(self, args):
        ...
        kwargs = get_lisa_from_pretrained_kwargs(args, torch_dtype)
        ...
        except (ValueError, torch.OutOfMemoryError) as exc:
            retry_kwargs = get_lisa_retry_load_kwargs(args)
            fallback_kwargs = get_lisa_cpu_fallback_kwargs(args)
            error_text = str(exc)
            retry_without_dispatch = (
                retry_kwargs is not None
                and "`.to` is not supported for `4-bit` or `8-bit` models" in error_text
            )
            if retry_without_dispatch:
                ...
                if quantization_config is not None:
                    if any(device in {"cpu", "disk"} for device in retry_kwargs.get("device_map", {}).values()):
                ...
                recoverable = (
                    fallback_kwargs is not None
                    and (
                        "Some modules are dispatched on the CPU or the disk" in error_text
        ...
        self.model.config.vision_tower = args.vision_tower
        self.model.config.mm_vision_tower = args.vision_tower
        self.model.get_model().config.vision_tower = args.vision_tower
        self.model.get_model().config.mm_vision_tower = args.vision_tower
        self.device = get_primary_device_from_model(self.model, self.device)
        ...
        if args.precision == "bf16" and self.device.type == "cuda":
        ...
        elif args.precision == "fp32" and self.device.type == "cuda":
    ...
    def segment(self, image_path, prompt):
        ...
        )["pixel_values"][0].unsqueeze(0).to(device=self.device)
